...
def plot_ood_from_pickle(data_origin, dummy_group_name=None):
    ...
    ood_plots.boxplot_from_nested_listdict(recall_dict, name='95% OOD recall', kind='bar',
                                           ...
                                           save_dir=os.path.join(ood_plot_dir_name,
                                                                 'ood_recall.png'),
                                           dummy_group_name=dummy_group_name,
                                           horizontal=True, ylim=None, legend=False,
                                           xlim=(0, 1.0),
                                           height=8, aspect=1, vline=0.05)

    name_dict = {'ece': 'ECE', 'roc_auc_score': 'ROC-AUC', 'accuracy': 'accuracy',
                 'brier_score_loss': 'Brier score', 'nll': 'NLL', 'log_loss': 'NLL'}
...
def plot_da_from_pickle():
            ...
            try:
                for metric in os.listdir(metrics_id_dir):
                    ...
                    with open(os.path.join(metrics_id_dir, metric), 'rb') as f:
                        metric_dict[metric][name] = {**metric_dict[metric][name], **pickle.load(f)}
    ...
    ood_plots.boxplot_from_nested_listdict(recall_dict, name='95% OOD recall', kind='bar',
                                           ...
                                           legend=False,
                                           xlim=None,
                                           height=3, aspect=3, vline=0.05)
    # just for legend
    ood_plots.boxplot_from_nested_listdict(recall_dict, name='Legend', kind='bar',
                                           x_name=' ',
                                           save_dir=os.path.join(ood_plot_dir_name,
                                                                 'legend.png'),
                                           horizontal=True, ylim=None, legend_out=True,
    ...
    name_dict = {'ece': 'ECE', 'roc_auc_score': 'ROC-AUC', 'accuracy': 'accuracy',
                 'brier_score_loss': 'Brier score', 'nll': 'NLL'}
...
def plot_perturbation_from_pickle(data_origin):
    ...
    auc_dict, recall_dict = dict(), dict()
    models = ['Single_NN', 'MC_Dropout (std)', 'MC_Dropout (entropy)',
              'NN_Ensemble (std)', 'NN_Ensemble (entropy)', 'NN_Ensemble_bootstrapped (std)',
              'NN_Ensemble_bootstrapped (entropy)', 'PPCA', 'AE']
    ...
    for method in models:
        ...
        with open(os.path.join(method_dir, 'perturb_recall.pkl'), 'rb') as f:
            recall_dict[name] = pickle.load(f)
    ood_plots.boxplot_from_nested_listdict(recall_dict, "perturbation 95% recall",
    ...
    ood_plots.boxplot_from_nested_listdict(auc_dict, "perturbation detection AUC", hline=0.5,
                                           ylim=None, figsize=(4, 4),
                                           save_dir=os.path.join(
                                               perturb_plot_dir_name, 'detect_AUC.png'),
                                           legend_args={'loc': 'center left',
                                                        'bbox_to_anchor': (1, 0.5)},
                                           xlim=None, showfliers=False, legend_out=True,
                                           legend=False)

    ood_plots.boxplot_from_nested_listdict(auc_dict, "perturbation detection AUC", hline=0.5,
                                           ylim=None, figsize=(10, 10),
                                           save_dir=os.path.join(
                                               perturb_plot_dir_name, 'legend.png'),
                                           legend_args={'loc': 'center left',
                                                        'bbox_to_anchor': (1, 0.5)},
                                           xlim=None, showfliers=False, legend_out=True,
                                           legend=True)
...
def confidence_performance_from_pickle(data_origin):
    ...
    metrics_to_use = [('AUC-ROC', metrics.roc_auc_score, (0.1, 0.88)),
    ...
    for metric_pretty_name, metric, ylim in metrics_to_use[:1]:
        plt.figure(figsize=(6, 6))
        analyzer.plot_incremental_metric(metric.__name__, title='', ylim=ylim, legend=True)
        plt.ylabel(metric_pretty_name)
        plt.tight_layout()
        plt.savefig(os.path.join(id_plot_dir_name, metric_pretty_name + "with_legend.png"),
                    dpi=300,
                    bbox_inches='tight', pad=0)
        plt.close()
...
if __name__ == '__main__':
    ...
    #plot_ood_from_pickle(data_origin=args.data_origin, dummy_group_name='-')
    plot_ood_from_pickle('MIMIC')
    # plot_da_from_pickle()
    ...
    # confidence_performance_from_pickle(data_origin='MIMIC')
