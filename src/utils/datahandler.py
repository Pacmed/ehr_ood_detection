...
def load_data_from_origin(origin: str = "MIMIC", ) -> dict:
    ...
    # MIMIC dataset has separated train/test splits
    if origin in MIMIC_ORIGINS:
        splits = [
            pd.read_csv(os.path.join(MAPPING_KEYS[origin]["data_folder"], csv_path), index_col=0)
            for csv_path in MAPPING_KEYS[origin]["split_paths"]
        ]
        ...
        data = pd.read_csv(MAPPING_KEYS[origin]["data_folder"], sep=MAPPING_KEYS[origin]["sep"])
        test_size, val_size = None, None
        shuffle = True

    with open(MAPPING_KEYS[origin]["feature_names_path"], "rb") as f:
        feature_names = pickle.load(f)

    try:
        other_groups = MAPPING_KEYS[origin]["other_groups"]
    except:
        other_groups = None

    try:
        ood_mapping = MAPPING_KEYS[origin]["ood_mapping"]
    except:
        ood_mapping = None
    ...
    return {
        ...
        "target_column": MAPPING_KEYS[origin]["target_name"],
